...
\newcommand{\institution}{University of Trieste}
  ...
  \begin{frame}[plain]
    \begin{tikzpicture}[remember picture,overlay]
      ...
      \node[anchor=west,text=Mist,text width=0.66\paperwidth,font=\large]
        at ([xshift=0.23\paperwidth,yshift=-1.5cm]current page.west) {#3};
    \end{tikzpicture}
  \end{frame}
}
...
\begin{frame}{Um processo é comportamento ao longo do tempo}
  \begin{columns}[T,onlytextwidth]
    \column{0.54\textwidth}
      ...
      \begin{itemize}
        \item \textbf{casos}: instâncias do processo
        \item \textbf{atividades}: passos do processo
        \item \textbf{eventos}: execuções gravadas das atividades
        \item \textbf{recursos}: pessoas ou sistemas envolvidos
...
\begin{frame}{Quatro perguntas, quatro ramos}
  ...
  \vspace{0.3cm}
  \begin{columns}[T,onlytextwidth]
    \column{0.23\textwidth}\textbf{Discovery}\\Revele o comportamento real.
    \column{0.23\textwidth}\textbf{Conformance Checking}\\compare o log e o modelo.
    \column{0.23\textwidth}\textbf{Process Enhancement}\\Adicione tempo, custo ou decisões.
    \column{0.23\textwidth}\textbf{Predictive Process Monitoring}\\Antecipe resultados, atrasos e riscos.
  \end{columns}
\end{frame}
...
\begin{frame}[shrink=5]{Mineração de processos não é um painel}
  \begin{tabular}{@{}>{\bfseries}p{0.20\textwidth}p{0.34\textwidth}p{0.36\textwidth}@{}}
    ...
    \midrule
    Unidade & Execuções & Casos e eventos ordenados\\
  ...
  \end{tabular}
  \vspace{0.5cm}
  \promptbox{Um Key Performance Indicator (KPI) de entrega piorou. Que perguntas sobre a sequência você faria antes de recomendar uma mudança?}
\end{frame}
...
\begin{frame}{Estruture uma análise útil}
  \begin{enumerate}
    ...
    \item \textbf{Successo}: Qual comportamento ou resultado deve mudar?
  \end{enumerate}
  \vspace{0.35cm}
  \begin{block}{Exemplo}
    ``Por que alguns pedidos estão atrasados?'' com PM: ``Quais caminhos, transferências e períodos de espera distinguem os pedidos entregues após a meta de cinco dias?''
  \end{block}
\end{frame}
...
\begin{frame}{O log de eventos mínimo viável}
  ...
  \begin{tabular}{@{}lllll@{}}
    \toprule
    Caso & Atividade & Data (hora) & Recurso & Valor\\
    \midrule
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \column{0.31\textwidth}
      \textbf{Data (timestamp)}\\Ordena os eventos no tempo.
  \end{columns}
  \vspace{0.35cm}
    ...
    \column{0.55\textwidth}
      \begin{tikzpicture}[x=1.15cm,y=1cm]
        ...
        \foreach \x/\lab/\col in {
          0.5/iniciar/Teal,2.0/suspender/Gold,3.0/retomar/Coral,4.8/concluir/Ink}
          {
...
\end{lstlisting}
  \vspace{0.15cm}
  \textbf{Antes de executar a descoberta:} verifique identificadores, timestamp, ordenação, duplicatas e o significado dos eventos.
\end{frame}
...
\begin{frame}{Flattening e encoding: duas camadas, duas decisões}
  \centering
  \begin{tikzpicture}[node distance=0.4cm and 0.7cm,scale=0.9,transform shape]
    \node[activity,fill=Mist,minimum width=3.1cm] (raw) {Log centrado\\em objetos};
    \node[activity,right=of raw,fill=Gold!35,minimum width=2.6cm] (flat) {Flattening};
    \node[activity,right=of flat,minimum width=2.6cm] (cases) {Traces baseados\\em casos};
    \node[activity,right=of cases,fill=Gold!35,minimum width=2.6cm] (enc) {Encoding\\do trace};
    \node[activity,right=of enc,minimum width=2.2cm] (var) {Variantes};
  ...
  \begin{columns}[T,onlytextwidth]
    \column{0.47\textwidth}
      \begin{block}{Flattening}
        Decide \textbf{quais eventos pertencem ao mesmo caso}. Mais amplo,
        anterior: escolhe a noção de caso (pedido, item, remessa\ldots) e
        cria convergência ou divergência antes de qualquer trace existir.
      \end{block}
    \column{0.47\textwidth}
      \begin{block}{Encoding}
        Decide \textbf{como um caso já definido é representado} como
        trace: fluxo de controle, ciclo de vida, tempo ou payload. Mais
        fino, posterior: molda as variantes depois que os casos já existem.
      \end{block}
  \end{columns}
  \vspace{0.3cm}
  \keyidea{Ambas são escolhas de modelagem, não padrões neutros: o
  flattening fixa o que conta como um caso; o encoding fixa como o
  histórico desse caso é lido.}
\end{frame}
...
\begin{frame}[shrink=8]{Representações de modelos}
  \centering
  \begin{columns}[T,onlytextwidth]
    \column{0.24\textwidth}
      ...
      \end{tikzpicture}
      \textbf{DFG}\\[0.15cm]
      \textit{Ponto forte:} visão geral rápida e legível\\
      \textit{Atenção a:} sem semântica completa de execução
    \column{0.24\textwidth}
      \centering
      ...
      \end{tikzpicture}
      \textbf{Árvore de processos}\\[0.15cm]
      \textit{Ponto forte:} estruturada e sound por construção\\
      \textit{Atenção a:} restringe a forma do modelo
    \column{0.24\textwidth}
      \centering
      \begin{tikzpicture}[scale=0.62,transform shape,
      ...
      \end{tikzpicture}
      \textbf{Rede de Petri}\\[0.15cm]
      \textit{Ponto forte:} concorrência e replay precisos\\
      \textit{Atenção a:} mais difícil para não especialistas
    \column{0.24\textwidth}
      \centering
      ...
      \end{tikzpicture}
      \textbf{BPMN}\\[0.15cm]
      \textit{Ponto forte:} notação de negócio familiar\\
      \textit{Atenção a:} a semântica varia conforme o uso
  \end{columns}
  \vspace{0.4cm}
...
\begin{frame}{Um protocolo de avaliação defensável}
  \centering
  \begin{tikzpicture}[node distance=1.5cm,scale=0.92,transform shape]
    ...
    \draw[flow] (train)--node[above,inner sep=3pt,font=\scriptsize]{ajustar}(valid);
    \draw[flow] (valid)--node[above,inner sep=3pt,font=\scriptsize]{selecionar}(pareto);
    \draw[flow] (pareto)--node[above,inner sep=3pt,font=\scriptsize]{revisar}(review);
    \draw[accentflow] (review)--node[above,inner sep=3pt,font=\scriptsize]{uma vez}(test);
  \end{tikzpicture}
  \vspace{0.55cm}
  relatório:
  \begin{itemize}
...
\begin{frame}[plain]
  \begin{tikzpicture}[remember picture,overlay]
    ...
    \node[text=Paper,font=\fontsize{20}{25}\selectfont\bfseries]
...
\begin{frame}{Referências — livro-texto principal}
  \begin{block}{Referência principal}
    van der Aalst, W.M.P. (2016). \textit{Process Mining: Data Science in
    Action} (2\textsuperscript{a} ed.). Springer.
  \end{block}
  \vspace{0.4cm}
  Este livro-texto, escrito pelo fundador da área de mineração de
  processos, é a referência principal para os conceitos abordados ao
  longo do curso: semântica do log de eventos, descoberta de processos,
  verificação de conformidade e aprimoramento de processos.
\end{frame}

\begin{frame}[shrink=8]{Referências — artigos selecionados}
  \small
  \begin{enumerate}
